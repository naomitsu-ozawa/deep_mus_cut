...
def show_images_in_grid(images, scores, columns=5, image_size=(3, 3), title=None):
    """
    images: 画像（Tensor）リスト
    scores: 各画像に対応する数値（例：分散値）
    columns: 1行に並べる画像数
    image_size: 1画像あたりの表示サイズ (inch単位, 横, 縦)
    title: タイトル（長文可）
    """
    import math
    rows = math.ceil(len(images) / columns)
    figsize = (columns * image_size[0], rows * image_size[1])
    ...
    for i, (img, score) in enumerate(zip(images, scores)):
        ...
        plt.axis("off")
        plt.title(f"{score:.1f}", fontsize=10)

    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.show()


def main(movie, model, cnn_model, num_images=10):
    if not os.path.exists(movie):
        print(f"Movie file {movie} does not exist.")
        return

    # ログレベルをWARNING未満に設定（INFOのみ通す）
    LOGGER.setLevel(logging.ERROR)  # 警告も出ないようにする
    ...
    # 推論
    cnn_results = cnn_model.predict(input_tensor, verbose=0)
    ...
    for t in thresholds:
        # しきい値以下のスコアを抽出
        candidates = [
            (img, score) for img, score in zip(result_images, focus_scores) if score <= t
        ]
...
if __name__ == "__main__":
    ...
    main(movie, model, cnn_model, num_images)
